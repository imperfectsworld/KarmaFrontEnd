...
export class ItemComponent {
  ...
  addItem() {
    ...
    this.getlocationservice
      .getLocation(this.formItem.geoCode)
      .subscribe((response) => {
        console.log(response);
        ...
        retryWhen((errors) =>
          errors.pipe(
            mergeMap((error) => {
              if (error.status === 429) {
                const retryAfter =
                  parseInt(error.headers.get('Retry-After'), 10) || 60;
                console.log(
  ...
  filterItems() {
    this.filteredItems = this.allItems.filter((item) => {
      const categoryMatches = this.selectedCategory
        ...
        : true;
      const conditionMatches = this.selectedCondition
  ...
  setModalContent(imageSrc: string, itemName: string) {
